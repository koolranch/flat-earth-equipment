...
export async function fetchRecommendations(payload: RecommendInput, signal?: AbortSignal): Promise<RecommendResponse> {
  const enabled = process.env.NEXT_PUBLIC_RECS_ENABLED ?? process.env.RECS_ENABLED;
  if (!enabled || enabled === '0') {
    return { ok: false, error: 'RECS_DISABLED' } as any;
  }
  ...
  if (!res.ok) {
    return { ok: false, error: await res.text() } as any;
  }
  return res.json();
}
