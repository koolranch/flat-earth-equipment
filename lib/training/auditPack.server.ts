...
/**
 * Builds the one-click OSHA audit pack PDF for a trainer: cover summary,
 * operator roster with certification + practical-evaluation status, evaluation
 * records, and every operator certificate appended.
 *
 * Shared by the authenticated trainer route and the public demo route (which
 * only ever passes the seeded demo trainer's id).
 */
...
export async function buildAuditPackPdf(trainerUserId: string): Promise<Uint8Array | null> {
  const svc = supabaseService();

  const { data: prof } = await svc.from('profiles').select('full_name, email').eq('id', trainerUserId).maybeSingle();

  // Same scoping as the roster: this trainer's orders -> seat claims -> learners.
  // Released seats are intentionally included: departed operators' records
  // must stay available for an OSHA audit.
  const { data: orders } = await svc.from('orders').select('id, course_id').eq('user_id', trainerUserId);
  ...
  const [{ data: learners }, { data: enrollments }, brand] = await Promise.all([
    ...
    managerSourceBrand(svc, trainerUserId),
  ]);
