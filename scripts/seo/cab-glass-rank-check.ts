/**
 * Cab-glass keyword SERP check via DataForSEO.
 *
 * - Writes dated JSON under scripts/seo/rank-snapshots/cab-glass/
 * - Prints delta vs most recent prior snapshot
 *
 * Usage: npx tsx scripts/seo/cab-glass-rank-check.ts
 * Cost:  ~$0.002 per keyword (live regular SERP)
 *
 * Money keywords = OEM PNs (highest intent) + brand/model/type + head/category.
 * Aligned with docs/projects/cab-glass-revenue/README.md.
 */
...
const OUT_DIR = path.resolve(process.cwd(), 'scripts/seo/rank-snapshots/cab-glass');

/** PN + model/type + head money keywords (purchase intent). */
const KEYWORDS = [
  // Head / category → /cab-glass
  'skid steer cab glass',
  'skid steer windshield',
  'compact track loader windshield',
  'bobcat cab glass',
  'bobcat door glass',
  // OEM part numbers (Buy Now heroes) → matching PDP
  '7120401',
  '6729776',
  '7219974',
  '7179045',
  ...
  '03586-00096',
  'V0511-33150',
  // Brand + model + glass type → PDP or hub deep-link
  'bobcat t650 door glass',
  'bobcat t770 door glass',
  'bobcat t190 door glass',
  'bobcat s650 door glass',
  'bobcat t650 windshield',
  'cat 259d door glass',
  'cat 259d windshield',
  'case sr175 door glass',
  'john deere 333e door glass',
  'kubota svl75 glass',
  'takeuchi tl10 door glass',
  'takeuchi tb230 door glass',
] as const;
...
async function main() {
  ...
  console.log(`Cab-glass rank check — ${date} — Google US top 100`);
  console.log(`Compare vs: ${priorLabel}\n`);
  console.log(
    `${'keyword'.padEnd(40)} ${priorLabel.slice(0, 10).padStart(10)} ${'Today'.padStart(8)}  URL`
  );
  ...
  for (const kw of KEYWORDS) {
    ...
    rows.push(row);
    if (row.error) {
      console.log(`${kw.padEnd(40)} ERROR: ${row.error}`);
    } else {
      const prev = priorPos(prior, kw);
      const marker = moveMarker(prev, row.position);
      console.log(
        `${kw.padEnd(40)} ${formatPos(prev).padStart(10)} ${(formatPos(row.position) + marker).padStart(8)}  ${row.url ?? '—'}  [top: ${row.topCompetitor}]`
      );
    }
