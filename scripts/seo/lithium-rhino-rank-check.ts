/**
 * Lithium batteries keyword SERP check via DataForSEO.
 *
 * - Money keywords + target winning URLs from constants/lithiumRhinoSeo.ts
 * - Writes dated JSON under scripts/seo/rank-snapshots/lithium-rhino/
 * - Prints delta vs most recent prior snapshot + target-URL mismatches
 *
...
import { LITHIUM_MONEY_KEYWORDS } from '../../constants/lithiumRhinoSeo';
...
async function main() {
  ...
  console.log(`Lithium batteries rank check — ${date} — Google US top 100`);
  ...
  const rows: Row[] = [];
  for (const { keyword, targetUrl, lane } of LITHIUM_MONEY_KEYWORDS) {
  ...
  console.log('Update docs/projects/lithium-batteries-revenue/STATUS.md from this snapshot.');
}
