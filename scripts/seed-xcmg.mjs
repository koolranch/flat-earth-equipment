...
const read = (p) => {
  ...
  console.log(`📋 Reading XCMG data from ${p}...`);
...
(async () => {
  console.log("🏗️ Starting XCMG lookup data seeding...");
  
  const cues = read("data/xcmg_model_cues.csv");
  const plates = read("data/xcmg_plate_locations.csv");
  const ranges = read("data/xcmg_model_serial_ranges.csv");
  
  await upsert("xcmg_model_cues", cues, "cue");
  await insertIgnoreDuplicates("xcmg_plate_locations", plates);
  await insertIgnoreDuplicates("xcmg_model_serial_ranges", ranges);
  
  console.log(`✅ Seeded cues=${cues.length}, plates=${plates.length}, ranges=${ranges.length}`);
  console.log("🎉 XCMG lookup data is ready!");
})();
