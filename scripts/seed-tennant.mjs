...
(async ()=>{
  console.log("🏗️ Starting Tennant lookup data seeding...");
  
  const cues = read('data/tennant_model_cues.csv');
  const plates = read('data/tennant_plate_locations.csv');
  const notes = read('data/tennant_model_serial_notes.csv');
  const ranges = read('data/tennant_model_serial_ranges.csv');
  
  await insertIgnoreDuplicates('tennant_model_cues', cues);
  await insertIgnoreDuplicates('tennant_plate_locations', plates);
  await insertIgnoreDuplicates('tennant_model_serial_notes', notes);
  await insertIgnoreDuplicates('tennant_model_serial_ranges', ranges);
  
  console.log(`✅ Seeded cues=${cues.length}, plates=${plates.length}, notes=${notes.length}, ranges=${ranges.length}`);
  console.log('🎉 Tennant lookup data is ready!');
})();
