/**
 * Convert JCB 333/H8243 quote stub → Buy Now. Keep sku/slug 333H8243.
 *
 * Magnasource: $468.87 (public page showed 1; vendor stock 24 on 2026-09-16)
 * Sell: ~5% under Mag (assembly rounding)
 *
 * Run: npx tsx scripts/convert-jcb-333-h8243-water-pump.ts
 */
...
const SKU = '333H8243';
const COMP_PRICE = 468.87;
const COMP_URL = 'https://www.magnasourceinc.com/itemdetail/JC333/H8243';
...
const LOCAL_IMAGE = path.resolve(
  process.cwd(),
  'public/images/parts/jcb-333-h8243-water-pump.jpg'
);
const STORAGE_PATH = 'jcb-333-h8243-water-pump.jpg';

const NAME = 'JCB 333/H8243 Water Pump Assembly';
const DESCRIPTION = [
  'Aftermarket water pump assembly for JCB equipment. Replaces OEM part number 333/H8243.',
  'Black cast housing with a side coolant outlet, a multi-hole mounting flange, a silver bearing face, and a white impeller. Gasket included.',
...
async function main() {
  ...
  if (!stripeProductId) {
    const product = await stripe.products.create({
      name: NAME,
      description:
        'Aftermarket JCB 333/H8243 water pump assembly. Side outlet, multi-hole flange, impeller, gasket included.'.slice(
          0,
          500
        ),
      images: [imageUrl],
  ...
  const { error: updErr } = await supabase
    ...
    .update({
      ...
      metadata: {
        ...
        product_type: 'water_pump',
        includes_gasket: true,
        competitor_prices: [
          {
            source: 'magnasource',
            price: COMP_PRICE,
