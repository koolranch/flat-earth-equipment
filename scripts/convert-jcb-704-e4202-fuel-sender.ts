/**
 * Convert JCB 704/E4202 quote stub → Buy Now. Keep sku/slug 704E4202.
 *
 * Magnasource: $142.37 in stock (17 on hand, 2026-09-16)
 * Sell: ~5% under Mag (assembly rounding under $300)
 *
 * Run: npx tsx scripts/convert-jcb-704-e4202-fuel-sender.ts
 */
...
const SKU = '704E4202';
const COMP_PRICE = 142.37;
const COMP_URL = 'https://www.magnasourceinc.com/itemdetail/JC704/E4202';
...
const LOCAL_IMAGE = path.resolve(
  process.cwd(),
  'public/images/parts/jcb-704-e4202-fuel-level-sender.jpg'
);
const STORAGE_PATH = 'jcb-704-e4202-fuel-level-sender.jpg';

const NAME = 'JCB 704/E4202 Fuel Level Sender';
const DESCRIPTION = [
  'Aftermarket fuel level sender for JCB equipment. Replaces OEM part number 704/E4202.',
  'Round mounting flange with a black pickup tube, a float arm with a black foam float, a 2-pin pigtail, and two metal hose nipples.',
...
async function main() {
  ...
  if (!stripeProductId) {
    const product = await stripe.products.create({
      name: NAME,
      description:
        'Aftermarket JCB 704/E4202 fuel level sender. Flange, pickup tube, float arm, 2-pin pigtail.'.slice(
          0,
          500
        ),
      images: [imageUrl],
  ...
  const { error: updErr } = await supabase
    ...
    .update({
      ...
      metadata: {
        ...
        product_type: 'fuel_level_sender',
        competitor_prices: [
          {
            source: 'magnasource',
            price: COMP_PRICE,
