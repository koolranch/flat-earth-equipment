...
(async ()=>{
  console.log("🏗️ Starting Lull lookup data seeding...");
  
  const cues = read('data/lull_model_cues.csv');
  const plates = read('data/lull_plate_locations.csv');
  const notes = read('data/lull_model_serial_notes.csv');
  const ranges = read('data/lull_model_serial_ranges.csv');
  
  await insertIgnoreDuplicates('lull_model_cues', cues);
  await insertIgnoreDuplicates('lull_plate_locations', plates);
  await insertIgnoreDuplicates('lull_model_serial_notes', notes);
  await insertIgnoreDuplicates('lull_model_serial_ranges', ranges);
  
  console.log(`✅ Seeded cues=${cues.length}, plates=${plates.length}, notes=${notes.length}, ranges=${ranges.length}`);
  console.log('🎉 Lull lookup data is ready!');
})();
