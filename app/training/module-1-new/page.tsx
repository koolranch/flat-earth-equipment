...
export default function Page() {
  ...
  const { done, markDone } = useModuleGate({
    courseId: 'forklift',
    moduleKey: 'm1-new', // Different key for test version
    initial: { osha: false, practice: false, cards: false, quiz: false }
  });
  
  React.useEffect(() => { track('lesson_start', { module: '1-new-mobile' }); }, []);
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-8 space-y-6">
      ...
      <div className='flex gap-2 mb-4 overflow-x-auto'>
        <button 
          className={`px-3 py-1.5 rounded-md border whitespace-nowrap ${tab==='osha'?'bg-white border-blue-500':'bg-slate-50 border-slate-200'}`} 
          onClick={() => setTab('osha')}
        >
          OSHA Basics <StatusDot state={done.osha ? 'done':'todo'} />
        </button>
        <button 
          className={`px-3 py-1.5 rounded-md border whitespace-nowrap ${tab==='practice'?'bg-white border-blue-500':'bg-slate-50 border-slate-200'}`} 
          onClick={() => setTab('practice')}
        >
          Practice <StatusDot state={done.practice ? 'done':'todo'} />
        </button>
        <button 
          className={`px-3 py-1.5 rounded-md border whitespace-nowrap ${tab==='flash'?'bg-white border-blue-500':'bg-slate-50 border-slate-200'}`} 
          onClick={() => setTab('flash')}
        >
          Flash Cards <StatusDot state={done.cards ? 'done':'todo'} />
        </button>
        <button
          className={`px-3 py-1.5 rounded-md border whitespace-nowrap ${tab==='quiz'?'bg-white border-blue-500':'bg-slate-50 border-slate-200'} ${!prereqsMet && 'opacity-50 cursor-not-allowed'}`}
