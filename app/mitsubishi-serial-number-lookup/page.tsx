...
export default function Page(){
  ...
  async function onSubmit(e: React.FormEvent){
    ...
    const res = await fetch("/api/mitsubishi-lookup",{
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10">
      <h1 className="text-3xl md:text-4xl font-semibold tracking-tight">Mitsubishi Forklift Serial Number Lookup</h1>
      <p className="mt-2 text-slate-600">
        Enter a Mitsubishi serial/PIN (and optional model). We'll show plate/stamp locations by family,
        infer family from model prefixes (FG/FD/FGC/FB), and decode the model year only when a true 17-character VIN/PIN is detected.
      </p>
      ...
      <form onSubmit={onSubmit} className="mt-6 space-y-4 bg-white rounded-2xl p-5 shadow">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input className="w-full rounded-xl border px-3 py-2" placeholder="e.g., FG25N A12345, FGC25N, or 17-char VIN/PIN" value={serial} onChange={(e)=>setSerial(e.target.value)} required />
            <p className="mt-1 text-xs text-slate-500">
              Tip: The name/serial (capacity/data) plate is typically in the operator compartment; the chassis/frame is also stamped with the serial.
            </p>
          </div>
        </div>
        <div>
          <label className="block text-sm font-medium mb-1">Model (optional)</label>
          <input className="w-full rounded-xl border px-3 py-2" placeholder="e.g., FG25N, FGC30N, FB35AN" value={model} onChange={(e)=>setModel(e.target.value)} />
        </div>
            ...
            <>
              ...
              <div className="mt-5">
                ...
                <ul className="list-disc pl-6 mt-2 text-slate-700">
                  ...
                  )) : <li>Check the name/serial plate in the operator area; locations vary by series.</li>}
                </ul>
              </div>
              ...
              <div className="mt-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <Link href="/quote" className="group block p-4 rounded-2xl border border-slate-200 hover:border-slate-300 transition-colors bg-white hover:bg-slate-50">
                    ...
                    <p className="text-sm text-slate-600 mt-1">Fast quotes for Mitsubishi parts</p>
                  </Link>
                  ...
                  <Link href="/parts" className="group block p-4 rounded-2xl border border-slate-200 hover:border-slate-300 transition-colors bg-white hover:bg-slate-50">
                    <div className="text-2xl mb-2">🔧</div>
                    <h4 className="font-medium text-slate-900 group-hover:text-black">Mitsubishi Parts</h4>
                  ...
                  <Link href="/battery-chargers" className="group block p-4 rounded-2xl border border-slate-200 hover:border-slate-300 transition-colors bg-white hover:bg-slate-50">
                    <div className="text-2xl mb-2">⚡</div>
                    <h4 className="font-medium text-slate-900 group-hover:text-black">Mitsubishi Battery Chargers</h4>
                    <p className="text-sm text-slate-600 mt-1">Compatible chargers for FB electric series</p>
                  </Link>
                </div>
              </div>
