...
export default function Page() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10">
      <h1 className="text-3xl md:text-4xl font-semibold tracking-tight">CASE Serial Number Lookup</h1>
      <p className="mt-2 text-slate-600">
        Enter a CASE serial/PIN (and optional model). We'll show plate/stamped frame locations by family,
        infer the family from common CASE cues (580N/590SN, SR/SV/TR/TV, CX, 521G–1121G, 650M–2050M, CX17C–CX60C),
      ...
      <form onSubmit={onSubmit} className="mt-6 space-y-4 bg-white rounded-2xl p-5 shadow">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input className="w-full rounded-xl border px-3 py-2" placeholder="e.g., 580N A12345, SR210, CX210D, 621G, 850M, or a 17-char VIN/PIN" value={serial} onChange={(e) => setSerial(e.target.value)} required />
            <p className="mt-1 text-xs text-slate-500">
              Tip: On N-Series backhoes the PIN plate is by the LH loader-cylinder pivot; many wheel loaders have a cab plate under the left seat.
            </p>
          </div>
        </div>
        <div>
          <label className="block text-sm font-medium mb-1">Model (optional)</label>
          <input className="w-full rounded-xl border px-3 py-2" placeholder="e.g., 580N, 590SN, SR210, TR310, CX145D, CX210D, 721G, 850M, CX60C" value={model} onChange={(e) => setModel(e.target.value)} />
        </div>
            ...
            <>
              ...
              <div className="mt-5">
                ...
                <ul className="list-disc pl-6 mt-2 text-slate-700">
                  ...
                  )) : <li>Check the data plate on the chassis/cab; many CASE machines also have a stamped frame serial.</li>}
                </ul>
              </div>
