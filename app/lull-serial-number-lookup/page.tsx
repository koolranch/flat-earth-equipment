...
type ApiResp = {
  ...
  match?: { normalizedModel:string|null };
...
export default function Page(){
  ...
  async function onSubmit(e: React.FormEvent){
    ...
    const res = await fetch('/api/lull-lookup',{ method:'POST', headers:{ 'content-type':'application/json' }, body: JSON.stringify({ serial, model: model || undefined }) });
    const json = await res.json(); setData(json); setLoading(false);
  }

  return (
    <div className="mx-auto max-w-3xl px-4 py-10">
      <h1 className="text-3xl md:text-4xl font-semibold tracking-tight">Lull Serial Number Lookup</h1>
      <p className="mt-2 text-slate-600">Enter your serial/PIN and optional model (e.g., 644E-42, 944E-42, 1044C-54 Series II). We'll show plate locations and any published serial-range notes.</p>
      ...
      <form onSubmit={onSubmit} className="mt-6 space-y-4 bg-white rounded-2xl p-5 shadow">
        <div>
          ...
          <input className="w-full rounded-xl border px-3 py-2" placeholder="e.g., 01600… • 644E-42 … • 1044C-54 …" value={serial} onChange={(e)=>setSerial(e.target.value)} required />
        </div>
        <div>
          <label className="block text-sm font-medium mb-1">Model (optional)</label>
          <input className="w-full rounded-xl border px-3 py-2" placeholder="644E-42 • 944E-42 • 1044C-54 Series II" value={model} onChange={(e)=>setModel(e.target.value)} />
          <p className="mt-1 text-xs text-slate-500">Tip: Record the serial exactly as shown on the plate.</p>
        </div>
          ...
          {'error' in data && data.error ? (
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
                <div className="rounded-xl bg-white p-3 border"><div className="text-slate-500">Model</div><div className="font-medium">{data.match?.normalizedModel || data.input?.model || '—'}</div></div>
              ...
              <div className="mt-5">
                ...
                <ul className="list-disc pl-6 mt-2 text-slate-700">
                  ...
                  )) : <li>Use the serial-number plate on the frame; on some models it's behind the right-front tire. Record the full serial.</li>}
                </ul>
              </div>
              ...
              {data.modelNotes?.length ? (
                <div className="mt-5">
                  <h3 className="font-semibold mb-2">Model-specific notes</h3>
                  <ul className="list-disc pl-6 text-slate-700">
