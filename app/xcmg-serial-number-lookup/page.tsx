...
type ApiResp = {
  ...
  parsed?: { family: string | null; series: string | null };
...
export default function Page() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setLoading(true); 
    setData(null);
    const res = await fetch("/api/xcmg-lookup", {
      method: "POST", 
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10">
      <h1 className="text-3xl md:text-4xl font-semibold tracking-tight">XCMG Serial Number Lookup</h1>
      <p className="mt-2 text-slate-600">
        Construction + MEWP. Enter your PIN/serial and optional model (e.g., XE210C, XD120, XG1930AC, XGS50E).
        We'll show where to find the identification plate/label and what to record.
      </p>
      ...
      <form onSubmit={onSubmit} className="mt-6 space-y-4 bg-white rounded-2xl p-5 shadow">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="md:col-span-2">
            ...
            <input className="w-full rounded-xl border px-3 py-2" placeholder="e.g., XE210C + PIN, XG1930AC, XGS50E" value={serial} onChange={(e) => setSerial(e.target.value)} required />
          </div>
          <div className="md:col-span-2">
            <label className="block text-sm font-medium mb-1">Model (optional)</label>
            <input className="w-full rounded-xl border px-3 py-2" placeholder="XE210C, ZL50GN, XS183, XD120, XP165U, GR165, XG1930AC, XGS50E, XGA16" value={model} onChange={(e) => setModel(e.target.value)} />
            <p className="mt-1 text-xs text-slate-500">
              Tip: XCMG manuals recommend recording both the machine PIN/serial and the engine serial, when available.
            </p>
          </div>
        </div>
          ...
          {"error" in data && data.error ? (
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
                ...
                <div className="rounded-xl bg-white p-3 border">
                  <div className="text-slate-500">Series</div>
                  <div className="font-medium">{data.parsed?.series || "—"}</div>
                </div>
                <div className="rounded-xl bg-white p-3 border">
              ...
              <div className="mt-5">
                ...
                <ul className="list-disc pl-6 mt-2 text-slate-700">
                  ...
                  )) : <li>Use the machine identification plate/label for the PIN/serial; record engine serial from the engine plate when accessible.</li>}
                </ul>
              </div>
              ...
              <section className="mt-8 text-sm text-slate-600">
                ...
                <a className="underline" href="/toro-serial-number-lookup">Toro</a> ·{" "}
