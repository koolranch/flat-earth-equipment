...
export type FinderBroom = {
  ...
  price: number;
  broomType: string;
  broomTypeLabel: string;
  partNumber: string;
  salesType: 'direct' | 'quote_only' | string;
  inStock: boolean;
};

export default function BroomFinder({ parts }: { parts: FinderBroom[] }) {
  ...
  const brands = useMemo(() => {
    const withModels = parts.filter((p) => p.models.length > 0);
    return Array.from(new Set(withModels.map((p) => p.brand))).sort();
  }, [parts]);
  ...
  const matches = useMemo(() => {
    ...
    return parts
      ...
      .sort(
        (a, b) =>
          a.broomTypeLabel.localeCompare(b.broomTypeLabel) ||
  ...
  return (
    <div className="bg-slate-900 rounded-2xl p-6 md:p-8 text-white">
      <h2 className="text-xl font-bold mb-1">Find brooms by machine</h2>
      <p className="text-slate-300 text-sm mb-5">
        Pick brand and model — results include main brooms and side brooms from the
        fitment catalog.
      </p>
        ...
        <div className="space-y-2">
          {matches.length === 0 ? (
            <p className="text-slate-300 text-sm">
              No brooms listed for this model yet —{' '}
              <Link
                href={`/quote?equipment=${encodeURIComponent(`${brand} ${model}`)}&notes=${encodeURIComponent('Broom / brush request')}`}
                className="underline text-orange-300"
              >
                request a quote
              </Link>{' '}
              and we&apos;ll source the correct broom.
            </p>
          ) : (
            matches.map((p) => {
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-slate-300">
                      {isBuyNow
                        ? 'In stock · Confirm size and filament'
                        : 'Request quote — confirm size and filament'}
                    </p>
                  </div>
