...
const EQUIPMENT_TYPES = [
  "IC Counterbalance (AX/BX)",
  "Electric Counterbalance (FB)",
  "Any"
];

export default function KomatsuLookupPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/komatsu-lookup", {
        method: "POST",
  ...
  const modelDecodeDisplay = useMemo(() => {
    if (!data?.modelDecode) return "—";
    
    const parts = [
      data.modelDecode.fuel_meaning,
      data.modelDecode.capacity_lbs,
      ...(data.modelDecode.designations || [])
    ].filter(Boolean);
  ...
  return (
    <>
      <Script 
        id="komatsu-lookup-structured-data"
      ...
      <div className="mx-auto max-w-3xl px-4 py-10">
        ...
        <div className="text-center mb-8">
          ...
          <p className="mt-4 text-lg text-slate-600 max-w-2xl mx-auto">
            Enter a Komatsu serial/PIN (and optional model). We'll show plate locations by family,
            decode the VIN year only when a true 17-character VIN/PIN is detected, and map basic IC model codes (FG/FD).
          </p>
        </div>
        ...
        <form onSubmit={onSubmit} className="mt-6 space-y-6 bg-white rounded-2xl p-6 shadow-lg border">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input 
                className="w-full rounded-xl border border-gray-300 px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500" 
                placeholder="e.g., FG25T-14 A12345, or 17-char VIN/PIN" 
                value={serial} 
                onChange={(e) => setSerial(e.target.value)} 
                required 
              />
              <p className="mt-2 text-xs text-slate-500">
                Tip (IC): data plate is on the hood to the right of the operator's seat; embossed SN on top of the left front fender.
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            <input 
              className="w-full rounded-xl border border-gray-300 px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500" 
              placeholder="e.g., FG25T-14, FG30HTUS, FB18" 
              value={model} 
              ...
              <>
                ...
                <div className="mb-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="rounded-xl bg-white p-4 border border-gray-200">
                      <div className="text-sm text-slate-500 mb-1">Model year (VIN/PIN only)</div>
                      <div className="font-medium text-gray-900">
                        {data.parsed?.vinYear ?? "—"}
                      </div>
                    </div>
                    <div className="rounded-xl bg-white p-4 border border-gray-200">
                      <div className="text-sm text-slate-500 mb-1">IC model code</div>
                      <div className="font-medium text-gray-900">
                        {modelDecodeDisplay}
                      </div>
                    </div>
                    ...
                    <ul className="space-y-3">
                      {plateTips.length ? plateTips.map((tip, i) => (
                        ...
                        <li className="text-slate-700">
                          Check the capacity/data plate; locations vary by series.
                        </li>
                      )}
                    </ul>
                  </div>
                ...
                <div className="mt-8 pt-6 border-t border-gray-200">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    >
                      ...
                      <div className="text-sm text-slate-600">Fast pricing on Komatsu parts</div>
                    </a>
                    ...
                    >
                      ...
                      <div className="font-medium text-gray-900 group-hover:text-orange-600">Komatsu Parts</div>
                    ...
                    >
                      ...
                      <div className="font-medium text-gray-900 group-hover:text-yellow-600">Komatsu Battery Chargers</div>
