...
export default function GenieCodeBrowser() {
  ...
  const categories = ['Safety', 'Electrical', 'Hydraulic', 'Control', 'Communication', 'Sensor', 'Drive', 'Engine', 'ECU', 'System'];
    ...
    <div>
      <div className="mb-6 flex flex-col sm:flex-row gap-3">
        <input
          ...
          className="flex-1 px-4 py-3 border-2 border-slate-300 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-purple-500 text-slate-900 placeholder:text-slate-500"
        />
        <select
          value={selectedCategory}
          onChange={(e) => setSelectedCategory(e.target.value)}
          className="px-4 py-3 border-2 border-slate-300 rounded-xl focus:ring-2 focus:ring-purple-500 text-slate-900"
        >
      ...
      <div className="space-y-2">
        {filteredCodes.map((item) => (
          <details key={item.code} className="bg-white border-2 border-slate-200 rounded-lg overflow-hidden hover:border-purple-400 transition-all">
            <summary className="cursor-pointer p-4 hover:bg-slate-50 flex items-center justify-between">
              <div className="flex items-center gap-3 flex-1">
                <span className="font-mono font-bold text-lg text-purple-600 flex-shrink-0">{item.code}</span>
