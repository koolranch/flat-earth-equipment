...
export default function MiniCheckoff({ locale = 'en', onComplete }: { locale?: 'en' | 'es', onComplete: () => void }) {
  ...
  const { assetsLoaded, isVisible, ref } = useLazyGameAssets({
    images: t.steps.map(s => s.img),
    backgrounds: [`${CDN_VIDEOS}/bg.png`]
  })
  ...
  return (
    <div ref={ref} className="relative aspect-video w-full max-w-md select-none overflow-hidden rounded-xl border bg-gray-100 shadow">
      {/* Loading state */}
      {!assetsLoaded && (
        <div className="w-full h-full flex items-center justify-center">
          <div className="text-gray-500 text-sm">Loading checkoff steps...</div>
        </div>
      )}
