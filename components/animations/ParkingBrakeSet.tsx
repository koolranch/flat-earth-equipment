...
export default function ParkingBrakeSet({ className }: { className?: string }) {
  ...
  React.useEffect(() => {
    ...
    async function load() {
      const res = await fetch("/training/animations/d2-parking-brake.svg", { cache: "force-cache" });
  ...
  return (
    <div
      className={className}
      aria-label="Parking brake set animation"
      role="img"
