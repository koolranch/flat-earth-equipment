...
// This is a wrapper around the existing exam logic to ensure it works with server-side initialization
export default function ExamWrapper({ mode, sessionData }: { 
  mode: 'resume' | 'create';
  sessionData?: any;
}) {
  ...
  useEffect(() => { 
    if (result && weak.length) { 
      (window as any)?.analytics?.track?.('exam_retake_tips_shown', { weak }); 
    } 
  }, [result, weak]);
  ...
  useEffect(() => {
    (async () => {
      try {
        if (mode === 'resume' && sessionData) {
          // Resume existing session
          setPaper({
            id: sessionData.paper_id,
            session_id: sessionData.id,
            items: sessionData.items,
            time_limit_sec: sessionData.remaining_sec,
            pass_score: undefined
          });
          setAnswers(sessionData.answers || new Array(sessionData.items.length).fill(-1));
          setRemaining(sessionData.remaining_sec);
          (window as any)?.analytics?.track?.('exam_resumed', { locale });
        } else {
          // Create new exam
          const gen = await api('/api/exam/generate', { 
            method: 'POST', 
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        await api('/api/exam/save-progress', {
          method: 'POST',
        ...
        (window as any)?.analytics?.track?.('exam_autosave', { remaining });
      } catch {}
    })();
  }, [remaining]);
  ...
  useEffect(() => {
    if (remaining === 0 && paper && !result) {
      (async () => {
        try {
          const r = await api('/api/exam/submit', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ session_id: paper.session_id, answers })
          });
          setResult(r);
          (window as any)?.analytics?.track?.('exam_timeout', { locale });
        } catch (e) {
          console.error(e);
        }
      })();
  ...
  async function submit() {
    ...
    setResult(r);
    (window as any)?.analytics?.track?.(r.passed ? 'exam_passed' : 'exam_failed', { 
      scorePct: r.scorePct, 
      locale 
    });
  }
      ...
      <main className="container mx-auto p-4 space-y-3">
        ...
        <div className="rounded-2xl border p-3">
          ...
          <div className="text-sm text-slate-700">
            {t('exam.score_label')}: {result.scorePct}% ({result.correct}/{result.total})
          </div>
        </div>
        
        {incorrect?.length > 0 && (
          <details className="rounded-2xl border p-3">
            <summary className="font-medium cursor-pointer">{t('exam.review_incorrect')}</summary>
            <ul className="list-disc pl-5 mt-2 text-sm">
              {incorrect.map((k: number) => (
                <li key={k}>
                  <span className="font-mono">Q{k + 1}</span>: {paper.items[k]?.question}
                </li>
              ))}
            </ul>
          </details>
          ...
          <section className="rounded-2xl border p-3">
            ...
            <ul className="text-sm grid gap-1">
              {weak.map(w => {
                ...
                return (
                  <li key={w.tag}>
                    <span className="font-mono">#{w.tag}</span> — {w.missed} missed
                    {R?.href && (
                      <>
                        {' · '}
        ...
        <div className="flex gap-2">
          <button 
            className="btn bg-[#F76511] text-white" 
            onClick={() => { location.reload(); }} 
            aria-label="Retake exam with new questions"
          >
            {t('exam.retake_exam')}
          </button>
          {incorrect?.length > 0 && (
            <a 
              ...
              href={`/training/exam/review?attempt=${result.attempt_id || ''}`}
            >
              {t('exam.review_incorrect')}
            </a>
          )}
          <a 
            className="btn border" 
            href="/records" 
            aria-label="View your certification records"
          >
            {t('exam.view_records')}
          </a>
        </div>
    ...
    <main className="container mx-auto p-4 space-y-3">
      <header className="sticky top-0 z-10 bg-white/80 backdrop-blur border-b p-2 flex items-center justify-between">
        ...
        <div className="text-sm">
          <span className="font-medium">{t('exam.time_remaining')}:</span> {mm}:{ss}
        </div>
      </header>
        ...
        <div className="grid gap-2">
          {item.choices.map((c: string, idx: number) => (
            <button
              ...
              className={`text-left border rounded-xl p-3 text-sm tappable ${
                answers[i] === idx ? 'border-slate-900' : ''
              }`}
              onClick={() => pick(idx)}
            >
              {c}
