...
**Weekly automation:** ran Mon 2026-09-14 (DataForSEO live regular, Google US top 100)  
...
| 1 Measurement | 🟡 | Weekly snapshot; **2 ranked** — JCB 150T held #66 (slipped from #62); **NEW** Takeuchi TL8 #85; Case still out; order attribution unverified |
| 2 Conversion | ⬜ | Hub/PDP UX largely shipped; sidebar→hub polish optional |
| 3 Model SERP wins | ⬜ | Protect JCB 150T first; watch Takeuchi TL8; recover Case TV370; Bobcat T650/T770 still out |
| 4 Expand | 🟡 | Merchant: 65 tracks; feed fix shipped — awaiting Center review clear |

## Rank snapshot (Google US) — 2026-09-14 vs Aug 31

| Keyword | Aug 31 | Sep 14 | Winning URL | Top competitor | Notes |
|---------|------:|-------:|-------------|----------------|-------|
...
| `jcb 150t tracks` | #62 | **#66** ↓ | `/parts/jcb-150t-rubber-track-320x86x48` | store.rubbertrax.com | **HELD** — correct model PDP (HTTP 200); best-ranked money KW |
| `takeuchi tl8 tracks` | out | **#85 NEW** | `/parts/takeuchi-tl8-rubber-track-320x86x52` | skidheaven.com | **WIN** — correct model PDP (HTTP 200) |
...
**Summary:** 2/29 ranked · 0 top 10 · 0 top 30 · 0 API errors · **1 NEW** (`takeuchi tl8 tracks` #85 on correct PDP) · **1 HELD** (`jcb 150t tracks` #62→#66, still correct PDP).  
Case TV370 + TV450 remain out (still out after Aug 10 LOST).  
Head terms do **not** land on `/rubber-tracks` hub. No wrong winning URLs (both ranked URLs are correct model PDPs).  
Full rows: `scripts/seo/rank-snapshots/rubber-tracks/2026-09-14.json`.  
Compare baseline on this branch: last committed prior = 2026-08-31 (interim weeks may lack a committed snapshot on this branch).
...
4. Optional: GSC + GA4 check on money PDPs `/parts/jcb-150t-rubber-track-320x86x48` (primary) and `/parts/takeuchi-tl8-rubber-track-320x86x52` (new foothold).

## Next action

**Exactly one (Phase 1):** Confirm Google indexing + GA4 organic landing coverage for the best-ranked money PDP `/parts/jcb-150t-rubber-track-320x86x48` (`jcb 150t tracks` #66, slipped from #62) — protect this foothold before title/H1 work on Takeuchi TL8 (#85 NEW), Case TV370 recovery, or never-ranked Bobcat terms. Both money PDPs HTTP 200; winning URLs correct (not hub, not wrong SKU).
...
| 2026-09-14 | Weekly pull: 1→2 ranked; JCB 150T HELD #62→#66 on correct PDP; `takeuchi tl8 tracks` NEW #85 on correct PDP; Case still out; 0 API errors; hub still out |
